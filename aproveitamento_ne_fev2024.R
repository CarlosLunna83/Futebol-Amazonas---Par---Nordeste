# Aproveitamento das equipes da Amazonas - Pará - Nordeste no mês de Fevereiro de 2024 (considerando só jogos oficiais)
...
aproveitamento_times_am_pa_ne_fev2024 <- times_am_pa_ne_geral_2024 %>%
  select(time, pontos_ganhos, meses, qtd_unidade) %>%
  filter(str_detect(meses, "fevereiro")) %>%
  group_by(time) %>%
...
aproveitamento_times_am_pa_ne_fev2024

view(unique(times_am_pa_ne_geral_2024$time))

view(aproveitamento_times_am_pa_ne_fev2024)

# Gráfico:

aproveitamento_times_am_pa_ne_geral_2024$time <- factor(aproveitamento_times_am_pa_ne_geral_2024$time, 
                                                        levels = unique(aproveitamento_times_am_pa_ne_geral_2024$time))

aproveitamento_times_am_pa_ne_geral_2024 %>%
...
# Jogos disputados por cada equipe em 2024:

jogos_disputados_am_pa_ne_fev2024 <- times_am_pa_ne_geral_2024 %>%
...
jogos_disputados_am_pa_ne_fev2024
