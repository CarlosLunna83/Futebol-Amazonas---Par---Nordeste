# Aproveitamento das equipes da Amazonas - Pará - Nordeste no mês de Agosto de 2024 (considerando só jogos oficiais)
...
aproveitamento_times_am_pa_ne_ago2024 <- times_am_pa_ne_geral_2024 %>%
  select(time, pontos_ganhos, meses, qtd_unidade) %>%
  filter(str_detect(meses, "agosto")) %>%
  group_by(time) %>%
...
aproveitamento_times_am_pa_ne_ago2024

view(unique(times_am_pa_ne_geral_2024$time))

view(aproveitamento_times_am_pa_ne_jun2024)

# Gráfico:

aproveitamento_times_am_pa_ne_ago2024$time <- factor(aproveitamento_times_am_pa_ne_ago2024$time, 
                                                        levels = unique(aproveitamento_times_am_pa_ne_ago2024$time))
...
aproveitamento_times_am_pa_ne_ago2024 %>%
  ...
  labs(title = "Aproveitamento times AM PA e NE em Agosto 2024",
       x = "Times",
       y = "Aproveitamento")

# Jogos disputados por cada equipe em Julho 2024:

jogos_disputados_am_pa_ne_jul2024 <- times_am_pa_ne_geral_2024 %>%
...
jogos_disputados_am_pa_ne_jul2024
