# Aproveitamento das equipes da Amazonas - Pará - Nordeste no mês de Abril de 2024 (considerando só jogos oficiais)
...
aproveitamento_times_am_pa_ne_abr2024 <- times_am_pa_ne_geral_2024 %>%
  select(time, pontos_ganhos, meses, qtd_unidade) %>%
  filter(str_detect(meses, "abril")) %>%
  group_by(time) %>%
...
aproveitamento_times_am_pa_ne_mar2024

view(unique(times_am_pa_ne_geral_2024$time))

view(aproveitamento_times_am_pa_ne_mar2024)

# Gráfico:

aproveitamento_times_am_pa_ne_abr2024$time <- factor(aproveitamento_times_am_pa_ne_abr2024$time, 
                                                        levels = unique(aproveitamento_times_am_pa_ne_abr2024$time))

aproveitamento_times_am_pa_ne_abr2024 %>%
  ...
  labs(title = "Aproveitamento times AM PA e NE em 2024",
       x = "Times",
       y = "Aproveitamento")

# Jogos disputados por cada equipe em Abril 2024:

jogos_disputados_am_pa_ne_abr2024 <- times_am_pa_ne_geral_2024 %>%
  select(time, meses, qtd_unidade) %>%
  filter(str_detect(meses, "marco")) %>%
  group_by(time) %>%
  summarise(jogos_disputados = sum(qtd_unidade)) %>%
  arrange(desc(jogos_disputados))

jogos_disputados_am_pa_ne_abr2024
